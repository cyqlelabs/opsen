...
func TestGeolocation_ErrorHandling(t *testing.T) {
	...
	if err == nil {
		t.Error("Expected error from failed geolocation API")
	}
}

// TestGeolocation_JSONParsing verifies JSON response parsing
func TestGeolocation_JSONParsing(t *testing.T) {
	...
	// Override geolocation URL temporarily
	oldURL := "https://ipapi.co/json/"

	config := Config{
		ServerURL:     server.URL,
		ClientID:      "geo-parse-test",
		WindowMinutes: 1,
	}
	...
	// Since we can't override the hardcoded URL easily, test the helper functions instead
	...
	lat := getFloatOrDefault(testData, "latitude", 0.0)
	if lat != 37.7749 {
		t.Errorf("Expected latitude 37.7749, got %f", lat)
	}

	city := getStringOrDefault(testData, "city", "")
	if city != "San Francisco" {
		t.Errorf("Expected city 'San Francisco', got '%s'", city)
	}

	_ = oldURL     // Use variable to avoid unused error
	_ = collector  // Use variable to avoid unused error
}
